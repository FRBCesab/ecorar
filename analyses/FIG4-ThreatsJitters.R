...
png(
  file       = file.path(path_figs, paste0(filename, "_", horizon, "_jitter.png")),
  width      = 12.00,
  height     =  7.00,
  units      = "in",
  res        = 600,
...
#'  -------------------------------------------------------------------------   @AddViolonPlot
      ...
      points(
        ...
        y = jitter(rep(which(classes == classe), length(tmp[which(tmp[ , "DR_class"] == classe), "Value"])), factor = factors[classe]),
        pch = 21,
        bg = paste0(color_classes[classe], "88"),
        col = "transparent",
        cex = 0.5
      )
